...
/*!
    ...
    This is the port of the Adafruit library. For original see below.
...
#include <MMA8451.h>
#include "ltrx_cpm.h" /* Configurable PIN: Delivered with SDK. */
#include "ltrx_spi.h" /* SPI Communication Delivered with SDK. */
...
_i2caddr = i2caddr;
  ...
  // enable 4G range
  MMA8451_writeRegister8(MMA8451_REG_XYZ_DATA_CFG, MMA8451_RANGE_4_G);
  // High res
...
static inline uint8_t MMA8451_i2cread(void) {
  ...
  TLOG(TLOG_SEVERITY_LEVEL__DEBUG, "i2cread from 0x%x ", _i2caddr);
  
}

static inline void MMA8451_i2cwrite(uint8_t x) {
  
  TLOG(TLOG_SEVERITY_LEVEL__DEBUG, "i2cwrite to Address(0x%x)  Value(0x%x)", _i2caddr, x);
